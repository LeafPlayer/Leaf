...
def iconset(asets_path, name, files)
  iconset_folder = File.join asets_path, "doc_#{name}.iconset"
...
def integrate_doc_types(info_plist_path, asets_path, icon_path)
  ...
  FT_DATA.each do |data|
    ...
    if File.directory?(icon_path)
      ...
      iconset asets_path, icon_name, icon_files
    end
